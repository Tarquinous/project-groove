    ...
    for i, buffToLoad in pairs(BuffsToLoad) do
        ...
        for j, buffedUnit in pairs(buffToLoad.unitClasses) do
            ...
            if existingBuff == nil then
                BuffLoader.Buffs[buffedUnit] =  buffToLoad.createFunction(buffedUnit)
            else
                BuffLoader.Buffs[buffedUnit] = stackFunctions(existingBuff, buffToLoad.createFunction(buffedUnit))
            end
        end
    end
end
